...
internal class AlbumContentsViewModel(
    ...
    private val knownItems = library.map { it.timeline.snapshot }.distinctUntilChanged()
    ...
    val state = combine(mutableState, knownItems, permission) { contents, libraryItems, access ->
        if (contents.location is AlbumLocation.Folder && access != LocalMediaPermissionMode.Full)
            contents.copy(items = emptyList(), total = 0, loaded = 0, loading = false, permissionRequired = true)
        else contents.copy(items = albumMediaProjection(contents.items,
            libraryItems, access == LocalMediaPermissionMode.Full))
    }.stateIn(viewModelScope, SharingStarted.Eagerly, AlbumContentsState())
    fun select(location: AlbumLocation?) { selection.value = location }
    fun refresh() { refreshes.update { it + 1 } }

    init {
        viewModelScope.launch {
            combine(sessionStore.session, selection, permission, refreshes) { session, location, mode, refresh ->
                LoadRequest(session, location, location is AlbumLocation.Folder && mode != LocalMediaPermissionMode.Full, refresh)
                ...
                if (needsPermission) {
                    mutableState.value = AlbumContentsState(location = location, permissionRequired = true)
                    return@collectLatest
                }
                mutableState.update { it.copy(loading = true) }
                try {
                    source.load(location, session.credentials).collect { batch ->
                        mutableState.update { it.copy(items = batch.items, total = batch.total, loaded = batch.loaded) }
                    }
                    mutableState.update { it.copy(loading = false) }
                } catch (cancelled: CancellationException) { throw cancelled }
                catch (_: Exception) { mutableState.update { it.copy(loading = false, failed = true) } }
            }
        }
    }
}
