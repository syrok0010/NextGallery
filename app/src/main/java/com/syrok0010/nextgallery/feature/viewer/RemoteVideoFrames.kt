...
internal class RemoteVideoFrames(
    ...
    override fun frames(uri: String): Flow<VideoFrameEvent> =
        flow {
                ...
                withTimeout(45_000) {
                    ...
                    val duration = MetadataRetriever
                        ...
                        .use { metadata ->
                            withTimeout(
                                8_000,
                            ) { metadata.retrieveDurationUs().awaitFrameResult() / 1000 }
                        }
                    val plan = VideoFilmstripProjection<Unit>()
                        .apply {
                            durationKnown(
                                duration,
                            )
                        }.state.positionsMillis
                        ...
                        .use { extractor ->
                            plan.forEachIndexed { index, position ->
                                val frame =
                                    withTimeout(
                                        5_000,
                                    ) { extractor.getFrame(position).awaitFrameResult() }
            ...
            when (failure) {
                is TimeoutCancellationException -> throw IOException(
    ...
    companion object {
        private val decoder = Mutex()
    }
}

private suspend fun <T> ListenableFuture<T>.awaitFrameResult(): T =
...
internal class UnsupportedVideoFrames(cause: Throwable) :
    IOException("Video format unavailable for frame extraction", cause)
