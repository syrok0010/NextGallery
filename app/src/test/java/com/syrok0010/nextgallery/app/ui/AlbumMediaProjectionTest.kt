...
class AlbumMediaProjectionTest {
    private val remote = MemoriesPhotoDto(42, 20, epoch = 1_728_000).toMediaItem(MediaId("shared-object"))
    private val local = remote.copy(dayId = 19, takenAtEpochSeconds = 1_641_600,
    ...
    @Test fun `folder keeps canonical cloud date and both copies without importing other media`() {
        ...
        val result = albumMediaProjection(listOf(local), listOf(remote, unrelated).associateBy { it.mediaId }, true).single()
        assertEquals(remote.dayId, result.dayId)
        assertEquals(remote.takenAtEpochSeconds, result.takenAtEpochSeconds)
        assertEquals(MediaAssetRef.LocalFirst(local.assetRef as MediaAssetRef.LocalContent,
            remote.assetRef as MediaAssetRef.MemoriesFile), result.assetRef)
    ...
    @Test fun `server album prefers known local copy only while permission allows it`() {
        ...
        assertTrue(albumMediaProjection(listOf(remote), library, true).single().assetRef is MediaAssetRef.LocalFirst)
        assertEquals(remote.assetRef, albumMediaProjection(listOf(remote), library, false).single().assetRef)
    }
}
