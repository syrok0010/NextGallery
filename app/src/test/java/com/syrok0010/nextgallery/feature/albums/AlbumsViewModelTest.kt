...
class AlbumsViewModelTest {
    @Test fun `remote failure preserves local results and refresh recovers independently`() =
        runTest {
    ...
    @Test fun `sign out cancels in flight source and prevents stale account publication`() =
        runTest {
            Dispatchers.setMain(StandardTestDispatcher(testScheduler))
            val store = ViewModelStore()
            try {
                ...
                store.put("vm", vm)
                runCurrent()
                assertTrue(vm.state.value.remote.loading)
                sessions.signOut()
                runCurrent()
                gate.complete(Unit)
                runCurrent()
