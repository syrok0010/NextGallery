...
class LocalMediaMetadataEnrichmentTest {
    @Test
    fun `new scan reuses persisted EXIF and invalidates changed generation metadata and version`() =
        runBlocking {
            val cache = Cache()
            var opens = 0
            suspend fun scan(rows: List<LocalMediaMetadata>, version: String = "v1") {
                val enrichment =
                    LocalMediaMetadataEnrichment(cache, mapOf("external_primary" to version)) {
                        opens++
    ...
    @Test
    fun `failed EXIF is retried and interrupted scan does not evict cached metadata`() =
        runBlocking {
            val cache = Cache()
            var attempts = 0
            repeat(2) {
                val enrichment =
                    LocalMediaMetadataEnrichment(cache, mapOf("external_primary" to "v1")) {
                        attempts++
                        null
                    }
                enrichment.load()
            ...
            assertEquals(1, cache.items.size)
            val successful =
                LocalMediaMetadataEnrichment(cache, mapOf("external_primary" to "v1")) { it }
            successful.load()
    ...
    @Test
    fun `unmounted volume is retained until it can be reconciled`() =
        runBlocking {
            val cache = Cache()
            val first =
                LocalMediaMetadataEnrichment(
                    cache,
                    mapOf("external_primary" to "v1", "card" to "v1"),
                ) { it }
            first.load()
