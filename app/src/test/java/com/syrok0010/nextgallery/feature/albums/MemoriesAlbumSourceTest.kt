...
class MemoriesAlbumSourceTest {
    @Test fun `authenticated list supports numeric strings and respects server hidden setting`() =
        runBlocking {
            Fixture(
                """{"albums_enabled":true,"show_hidden_albums":false}""",
                """[{"album_id":1,"name":"Отпуск","count":"7","user_display":"Анна"},{"album_id":2,"name":".hidden","count":0}]""",
            ).use { fixture ->
                ...
                assertEquals(
                    listOf(
                        "GET /nextcloud/apps/memories/api/config HTTP/1.1",
                        "GET /nextcloud/apps/memories/api/clusters/albums HTTP/1.1",
                    ),
                ...
                assertTrue(
                    fixture.requests.all { lines ->
                        lines.any {
                            it == "Authorization: ${Credentials.basic("fixture", "password")}"
                        }
                    },
                )
            }
    ...
    @Test fun `disabled feature does not request album list`() =
        runBlocking {
    ...
    @Test fun `http failure is not a successful empty catalog`() =
        runBlocking {
    ...
    private class Fixture(val config: String, val albums: String, val code: Int = 200) :
                ...
                while (!server.isClosed) {
                    ...
                    socket.use {
                        ...
                        it.getOutputStream().apply {
                            write(
                                "HTTP/1.1 ${if (isConfig) 200 else code} Fixture\r\nContent-Type: application/json\r\nContent-Length: ${bytes.size}\r\nConnection: close\r\n\r\n"
                                    .toByteArray(),
                            )
                            write(bytes)
                            flush()
        ...
        suspend fun load() =
            MemoriesAlbumSource(
                NextcloudTransport(Json),
            ).load(
                AccountCredentials(
                    "http://127.0.0.1:${server.localPort}/nextcloud",
                    "fixture",
                    "password",
                ),
            )
