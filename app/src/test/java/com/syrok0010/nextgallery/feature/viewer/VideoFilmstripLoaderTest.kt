...
class VideoFilmstripLoaderTest {
    @Test fun retryCancelsPreviousExtractionAndCloseReleasesCurrentRequest() =
        runTest {
            var started = 0
            var cancelled = 0
            val loader = VideoFilmstripLoader(
                VideoFrameProvider {
                    flow {
                        started++
                        try {
    ...
    @Test fun failedExtractionCanRetryWithoutLosingAbilityToScrub() =
        runTest {
            var attempts = 0
            val loader = VideoFilmstripLoader(
                VideoFrameProvider {
