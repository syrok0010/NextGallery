...
class AlbumCatalogTest {
    @Test fun `same names and matching covers do not merge containers`() {
        val local = localAlbumSummaries(sequenceOf(
        ...
        val remote = remoteAlbumSummary(Json.parseToJsonElement("""{"album_id":1,"name":"Отпуск","count":"3","cover":"2","cover_etag":"v2","user":"anna"}""").jsonObject)
    ...
    @Test fun `cover falls back to last added and unknown counts stay unknown`() {
        val result = remoteAlbumSummary(Json.parseToJsonElement("""{"album_id":3,"name":"Empty","cover":0,"last_added_photo":"42","last_added_photo_etag":"v3","count":null,"user_display":"Анна"}""").jsonObject)
        assertEquals(AlbumCover.Remote(42, "v3"), result.cover)
        assertNull(result.count)
        assertEquals("Анна", result.detail)
        assertNull(remoteAlbumSummary(Json.parseToJsonElement("""{"album_id":4,"name":"Empty","last_added_photo":0,"count":0}""").jsonObject).cover)
    }
}
