...
class UnifiedTimelineProjectionPersistenceTest {
    ...
    @Test
    fun mergedAliasesAndLocalMediaIdSurviveDatabaseReopen() =
        runBlocking {
            val local = item(
            ...
            val identifiedRemote = remote.copy(
                mediaId = resolvedRemote.mediaIds.getValue(remote.sourceIdentity()),
            )
            val afterRestart = UnifiedTimelineProjection()
                .replaceRemoteSnapshot(remoteSnapshot(identifiedRemote))

            assertEquals(
                MediaId("published-local"),
                requireNotNull(afterRestart.snapshot).items.single().mediaId,
            )
        }

    @Test
    fun aliasConflictSurvivesDatabaseReopenForDiagnostics() =
        runBlocking {
            val first = item(
    ...
    private fun remoteSnapshot(item: MediaItem) =
        TimelineSnapshotAssembler.assemble(
    ...
    private fun MediaItem.identityCandidate() =
