...
class TimelineCacheRepositoryOfflineTest {
    ...
    private val imageCache get() = com.syrok0010.nextgallery.feature.images.RemoteImageCache(
        database,
        thumbnailFileStore,
    )
    ...
    @Test
    fun cachedSnapshotContainsOnlyMaterializedCloudObjects() =
        runBlocking {
    ...
    @Test
    fun clearingCloudCachePreservesDeviceProjectionAndReusableIdentity() =
        runBlocking {
                ...
                listOf(
                    ...
                    MediaIdentityCandidate(
                        source = MediaSourceIdentity(
                            MediaSourceKind.Memories,
                            REMOTE_ONLY_FILE_ID.toString(),
                        ),
            ...
            assertEquals(
                LOCAL_CONTENT_URI,
                (restoredLocal.assetRef as MediaAssetRef.LocalContent).contentUri,
            )
            ...
            val remoteOnlySource =
                MediaSourceIdentity(MediaSourceKind.Memories, REMOTE_ONLY_FILE_ID.toString())
            ...
            assertEquals(
                MediaId("remote-only-after-login"),
                reloggedRemoteOnly.mediaIds.getValue(remoteOnlySource),
            )
        }
    ...
    private fun MediaItem.sourceIdentity(): MediaSourceIdentity =
        when (val asset = assetRef) {
            is MediaAssetRef.MemoriesFile -> MediaSourceIdentity(
                MediaSourceKind.Memories,
                asset.photoFileId.toString(),
            )

            is MediaAssetRef.LocalContent -> MediaSourceIdentity(
    ...
    private suspend fun loadThumbnailKeys(item: MediaItem) =
        imageCache.loadThumbnailKeys(
    ...
    private fun thumbnailKey(item: MediaItem) =
        ThumbnailKey(
            accountScope = ACCOUNT_SCOPE,
            fileId = FILE_ID,
            width = THUMBNAIL_SIZE,
