...
class MemoriesVideoDiscoveryTest {
    @Test fun `disabled VOD never requests transcode`() =
        runBlocking {
            ...
            assertTrue(
                discovery
                    .qualities(
                        RemoteVideoOriginal(42, "https://cloud.example/nextcloud"),
                        "client123",
                    ).isEmpty(),
            )
            assertEquals(
                listOf("/nextcloud/apps/memories/api/config"),
                requests.map { it.url.encodedPath },
            )
        }

    @Test fun `config and manifest use authenticated subpath contract`() =
        runBlocking {
            val requests = mutableListOf<Request>()
            val result = discovery(
                requests,
                false,
            ).qualities(RemoteVideoOriginal(42, "https://cloud.example/nextcloud"), "client123")
            assertEquals(listOf("Auto", "360p"), result.map { it.label })
            assertEquals(
                "/nextcloud/apps/memories/api/video/transcode/client123/42/index.m3u8",
    ...
    private fun discovery(
        ...
        disabled: Boolean,
        status: Int = 200,
    ): MemoriesVideoDiscovery {
        val transport = NextcloudTransport(Json { ignoreUnknownKeys = true }, OkHttpClient())
        val client = transport.baseClient
            .newBuilder()
            .addInterceptor(
                AuthenticatedVideoSource(transport) {
                    AccountCredentials("https://cloud.example/nextcloud", "alice", "secret")
                },
                ...
                Response
                    ...
                    .code(if (config) 200 else status)
                    .message("fixture")
                    .body(
                        (
                            if (config) {
                                """{"version":"7","unrelated_setting":true,"vod_disable":$disabled}"""
                            } else {
