...
Таймлайн и содержимое альбома используют `MediaCollectionScreen`: одну сетку с днями, сохранением позиции, быстрым скроллом и переходами в viewer. Интерфейс принимает упорядоченные слоты (в том числе ещё не загруженные), callbacks догрузки и оформление экрана. Это сохраняет виртуализацию таймлайна без фиктивного TimelineSnapshot для альбома. Состав, сортировка и обогащение альбома остаются в AlbumContentsViewModel; общая сетка не добавляет элементы медиатеки. Отдельные сетки потребовали бы дублировать восстановление и координаты переходов, а общая ViewModel смешала бы разные правила загрузки. Диагностика обоих источников остаётся в меню экрана.

TimelineWorkflow получает remote-source interface, local flow factory и scope сессии. Только он меняет snapshot и day loading states. TimelineRepository наблюдает session и создаёт workflow в дочернем scope; AlbumCatalogRepository независимо поддерживает каталог. Repositories живут в процессе приложения, но работа и результаты ограничены авторизованной сессией. Поэтому индексация не зависит от наличия PhotosScreen в композиции, а диагностика и другие экраны не получают чужой ViewModel. TimelineViewModel предоставляет экрану состояние и команды таймлайна; AlbumsViewModel — каталог. Разрешения публикуются общим координатором, выход принадлежит SessionViewModel. Viewport policy стала чистой функцией без обратного host. Refresh начинает новое поколение, отменяет прежнюю hydration и явно сбрасывает failed days.
