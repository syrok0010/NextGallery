...
Drag внутри карточки поглощает gesture и отправляет silent seek только активной
playback session. Player временно замолкает и останавливается; после отпускания
сохраняет найденную позицию и прежний play/pause intent. Если видео ещё не было
запущено или стояло на паузе, оно остаётся на паузе; фон отменяет возобновление.
Mute preference сохраняется. TalkBack получает описание состояния и seek action.
