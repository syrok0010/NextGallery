...
Лента имеет постоянную длину в две ширины viewport независимо от duration; число
кадров остаётся 24. Пустые поля по пол-экрана позволяют совместить начало и конец
видео с неподвижным центральным указателем. Коэффициент длины выбран как стартовый
для проверки удобства: длинные видео не увеличивают путь прокрутки.

Drag прокручивает ленту, включая инерцию Compose, и отправляет silent seek только
активной playback session. Внешняя навигация filmstrip при этом отключена. После
