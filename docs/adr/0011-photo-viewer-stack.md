...
- `MediaViewerPage` размещает общий preview для фото и видео; поверх него отображается оригинал фото с zoom либо кадры Media3 через `ContentFrame`. `ViewerChrome` владеет центральной кнопкой и панелью видеоконтролов вместе с topbar и filmstrip. Общий запрет chrome скрывает их при dismiss; перенос всего surface в chrome отклонён, поскольку изображение должно продолжать трансформироваться. Fullscreen-эффекты принадлежат владельцу playback и не зависят от видимости chrome. В fullscreen topbar и filmstrip скрыты, видеоконтролы доступны.
...
Критерии: умеренный рост памяти и длины ленты, независимость от обычного playback и однозначный drag.
Нажатое local video раскрывается внутри существующего filmstrip item; `MediaId`
не меняется. `VideoFilmstripProjection` задаёт равномерные позиции, progressive/
...
на приложение, кадр не более 160 px по длинной стороне. Используется
`OPTION_CLOSEST_SYNC`: миниатюрам достаточно ближайшего ключевого кадра,
что избегает точного декодирования каждой позиции. При редких ключевых кадрах
картинки могут повторяться; выборочного уточнения пока нет. Точный seek player
от этого выбора не зависит. Работа отменяется при
...
Число кадров рассчитывается по формуле Samsung Gallery:
`N = max(3, int(7 × log10(max(1, durationMillis / 1000))))`,
где деление на 1000 целочисленное. Ширина карточки — `59 dp × N`.
Например, 10 секунд дают 7 кадров, минута — 12, час — 24. Это заменяет
первоначальные фиксированные 24 кадра и два viewport: короткие видео требуют
меньше извлечений, длинные увеличивают ленту логарифмически, а не линейно.
Жёсткого максимума числа кадров нет. Позиции по-прежнему покрывают интервал
от нуля до duration − 1 мс; формула Samsung не меняет диапазон seek.
Указатель закреплён в центре viewport общей filmstrip.
...
Одновременно строится одна remote-лента: N последовательных запросов кадров
по тому же плану, что у локальной ленты;
...
### Компактный chrome просмотра

Название и дата центрированы относительно экрана, стрелка Back выровнена по
середине блока с симметричным отступом справа. Play/Pause находится в центре
кадра; при буферизации кнопка остаётся доступной с кольцом загрузки. Повторная
кнопка Play в нижней панели исключена, чтобы освободить место для перемотки и
качества. Панель имеет боковые отступы 20 dp, скруглённый фон высотой 36 dp,
иконки 18 dp и область управления высотой не менее 48 dp. В ней остаются время,
seek, доступные качества, звук и fullscreen. Меню отмечает выбранное качество;
при отсутствии вариантов качества соответствующая кнопка не показывается.
