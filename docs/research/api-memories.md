...
Проверено по тому же checkout Memories (`abea4165`):
`ClustersBackend/AlbumsBackend.php::transformDayQuery`, `Db/AlbumsQuery.php::getIfAllowed`,
`Controller/DaysController.php`, `Db/FsManager.php::getFile` и `getAlbumFile`.

- `GET /apps/memories/api/days?albums=<cluster_id>` возвращает индекс дней выбранного
  альбома; `GET /apps/memories/api/days/<ids>?albums=<cluster_id>` — элементы этих дней.
  Фильтр обязателен в обоих запросах. `cluster_id` — строка `user/name`, не числовой
  `album_id`; её нужно передавать как кодируемый query parameter, включая Unicode,
  пробелы, `/` и `&`.
- Начальный индекс может содержать `detail`. Неполные дни запрашиваются дополнительно;
  клиент загружает метаданные порциями до завершения или ухода со страницы. Это не
  viewport-driven загрузка метаданных; сами изображения создаются ленивой сеткой.
- Авторизованный доступ к preview/stream использует обычный `fileId`: FsManager
  после собственного файлового дерева проверяет доступ через albums collaborators.
  Это не поддержка анонимных публичных ссылок и не гарантия, что удалённый оригинал
  останется доступен после загрузки метаданных.
- Содержимое проходит через общий identity registry. Счётчик загрузки относится
  к записям источника, а сетка и viewer sequence содержат уникальные `MediaId`.
  Листание ограничено открытым альбомом и не подмешивает остальной timeline.
- Локальное содержимое выбирается точным равенством `VOLUME_NAME` и `RELATIVE_PATH`,
  без вложенных папок, pending и trashed rows. Повторного EXIF-сканирования нет:
  неизменённые файлы используют каноническое время из локальной проекции индекса;
  для ещё не индексированных применяется существующий MediaStore fallback.
  Общая медиатека обогащает состав альбома известными каноническими метаданными
  и доступными копиями: LocalFirst сохраняет локальное чтение и cloud fallback.
- Содержимое хранится в памяти и перечитывается при открытии/ручном обновлении.
  Смена альбома, logout и потеря полного локального разрешения отменяют загрузку.
  Автоматическое наблюдение изменений открытой папки и offline-каталог не реализованы.
